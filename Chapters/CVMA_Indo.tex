%Efficiently computable underlying finite field arithmetic is considered as one of the major bottlenecks for faster pairing operation.
...
The paper is organized into 5 sections with relevant subsections.
...
%----------------------------------------------------------------
\subsubsection{Field Arithmetic  of $\FPSN$}
...
The squaring is optimized by using Devegili et al.'s \cite{devegili2006multiplication} complex squaring technique which costs $2M_p+4A_p+2m_\alpha$ for one squaring operation in $\FPT$. 
...
The optimal-ate pairing on KSS-16 elliptic curve is given by Zhang et al. \cite{zhang_L12} and presented by the following map.
\begin{eqnarray*}
...
\begin{algorithm}[!h]
	...
	\nl \For{$i = \lfloor \log_2 (u)\rfloor $ {\bf downto} $1$} {
		\nl $f\leftarrow f^2\cdot l_{T,T}(P)$, $T\leftarrow [2]T$ \Comment*[r]{see \eqref{sparse_dbl}}
		\nl \If{$u[i]=1$} {
			\nl $f\leftarrow f\cdot l_{T,Q}(P)$, $T\leftarrow T+Q$ \Comment*[r]{see \eqref{sparse_add}}}
		\nl \If{$u[i]=-1$} {
			\nl $f\leftarrow f\cdot l_{T,-Q}(P)$, $T\leftarrow T-Q$\Comment*[r]{see \eqref{sparse_add}}} } 
...
\end{enumerate}
%-----------------------------------------------------------------------------------
\subsection{$\FPFR$ arithmetic in Type-II towering}
...
\renewcommand{\baselinestretch}{1.0}
%---------------------------------------------------------------------------------------------
\subsubsection{Multiplication in $\FPFR$ using CVMA}
...
\subsubsection{Squaring in $\FPFR$ using CVMA}
...
%------------------------------------------------------------------------------------------
\subsubsection{Frobenius mapping in $\FPFR$ using CVMA}
...
%---------------------------------------------------------------------------------------------
\subsubsection{Optimized  $\FPFR$ Inversion using CVMA}
...
%-------------------------------------------------------------------------------------------------------
\subsubsection{Calculation over $\mathbb{F}_{p^2}$ based on towering \eqref{towering_2}}
...
\subsubsection{Frobenius mapping in $\FPSN$ using CVMA}\label{fobenius_map}
...
%	\caption{Vector representation of $Q = (x_Q,y_Q) \in \g2 \subset E(\mathbb{F}_{p^{16}})$}
	...
	\resizebox{\columnwidth}{!}{
		...
		\label{tab_Q}
\end{table}
%Here, $a_j \in \Fp$, where $j={0, 1, 2, \cdots,15}$.
...
We start this paragraph by presenting the 7-sparse multiplication of the elliptic curve doubling of $T+T = R(x_R, y_R)$ given in \cite{aranha2011faster, grewal2012efficient}. 
...
&l_{T,T}(P)   = y_P+F \beta +E\beta \gamma&   \label{sparse_dbl}
\end{alignat}
...
\begin{algorithm}[!htp]
	...
	\nl $z^{-2} \leftarrow z^{-2}\delta$  \Comment*[r]{used during ECD in \eqref{E_bar_prime}; $1M_{p^4}$}
...
\begin{algorithm}[!h]
	...
	\nl Pre-compute $\bar{Q'},\bar{P},y_{P}^{-1}, z^{-2}, z^{-2}\delta$ \Comment*[r]{see Alg. \ref{pre_calc_Algo}}
	
	\nl $f \leftarrow 1, \bar{T'} \leftarrow \bar{Q'}$\;
	\nl \For{$i = \lfloor \log_2 (u)\rfloor $ {\bf downto} $1$} {
		\nl $f\leftarrow f^2\cdot \bar{l}_{\bar{T'},\bar{T'}}(\bar{P}) $, $ \bar{T'} \leftarrow [2] \bar{T'} $ \Comment*[r]{apply Alg. \ref{algo_sparse_mul_kss16} }
		\nl \If{$u[i]=1$} {
			\nl $f\leftarrow f\cdot \bar{l}_{\bar{T'},\bar{Q'}}(\bar{P})$, $ \bar{T'} \leftarrow  \bar{T'} + \bar{Q'} $ \Comment*[r]{apply Alg.\ref{algo_sparse_mul_kss16} to solve \eqref{ps_8_add_twist}}}
		\nl \If{$u[i]=-1$} {
			\nl $f\leftarrow f\cdot \bar{l}_{\bar{T'},\bar{Q'}}(\bar{P})$, $ \bar{T'} \leftarrow  \bar{T'} - \bar{Q'} $\Comment*[r]{ apply Alg.\ref{algo_sparse_mul_kss16} to solve \eqref{ps_8_add_twist}}}} 
	\nl $Q_1\leftarrow [u]\bar{Q'}$  \Comment*[r]{here $Q_1 = \bar{T'}$}
	\nl $Q_2\leftarrow [p]\bar{Q'}$ \Comment*[r]{Skew Frobenius map Sec. \ref{sec_tskew_fm}}
	\nl $f\leftarrow f\cdot l_{Q_1,Q_2}(\bar{P})$ \Comment*[r]{ Alg.\ref{algo_sparse_mul_kss16}}
	\nl $f_t\leftarrow f^{p^3}$   \Comment*[r]{Forbenius map of $p^3$}
	\nl $f\leftarrow f\cdot f_t$ \Comment*[r]{Alg.\ref{algo_sparse_mul_kss16}}
	\nl $f\leftarrow f\cdot l_{\bar{Q'},\bar{Q'}}(\bar{P})$ \Comment*[r]{ Alg.\ref{algo_sparse_mul_kss16}}
...
The parameter is chosen according to \cite{sylvain_new_param}'s suggestion for to make DLP size secure enough against exTNFS \cite{kim_ecdlp} as is shown in Table \ref{parameters}.
